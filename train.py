...
parser.add_argument('--IPC', default=0, type = int,
...
parser.add_argument('--scale_pos', default = 2, type=float,
    help = 'MS scale for positives'
)
parser.add_argument('--scale_neg', default = 50, type=float,
    help = 'MS scale for negatives'
)
parser.add_argument('--margin', default = 0.01, type=float,
    help = 'ST margin'
)
parser.add_argument('--lamda', default = 20, type=float,
    help = 'ST scale'
)
parser.add_argument('--gamma', default = 1./0.1, type=float,
    help = 'ST gamma'
)
parser.add_argument('--tau', default = 0.4, type=float,
    help = 'ST tau for weight reg'
)
...
for i in range(100):
    images[i]=(1/(2.25*2.5))*images[i]+0.5
    plt.imsave('sop_samples/1'+str(i)+'.jpg',np.transpose(images[i].cpu().detach().numpy(), (1,2,0)))
'''
...
if args.loss == 'Proxy_Anchor':
    ...
    criterion = losses.MultiSimilarityLoss(thresh = args.thresh, epsilon = args.epsilon, scale_pos = args.scale_pos, scale_neg = args.scale_neg).cuda()
    ...
    criterion = losses.ArcFaceLoss(nb_classes = nb_classes, sz_embed = args.sz_embedding).cuda()
elif args.loss == 'SoftTriple':
    criterion = losses.SoftTripleLoss(margin = args.margin, lamda =args.lamda, gamma=args.gamma , tau=args.tau ,nb_classes = nb_classes, sz_embed = args.sz_embedding).cuda()
elif args.loss == 'LiftedStructure':
    criterion = losses.LiftedStructureLoss().cuda()
elif args.loss == 'Margin':
    criterion = losses.MarginLoss().cuda()
...
if args.loss == 'Proxy_Anchor':
    ...
    #this used to make alpha learnable
    #param_groups.append({'params': criterion.alpha, 'lr':float(args.lr) * 500})

if args.loss == 'SoftTriple':
    param_groups.append({'params': criterion.fc, 'lr':float(args.lr) * 100})
...
for epoch in range(0, args.nb_epochs):
    ...
    if(epoch >= 0):
        ...
        if best_recall[0] < Recalls[0]:
            ...
            # that's added in order to save the proxies with the best_recall in a file
            #if not os.path.exists('proxies'):
            #    os.makedirs('proxies')
            #torch.save(P, 'proxies/proxies.pt')
...
if args.loss == 'Proxy_Anchor':
    with open('{}/{}_{}_{}_mrg{}_alpha{}.txt'.format(LOG_DIR, args.dataset, args.model, args.loss, args.mrg, args.alpha), 'w') as f:
        for i in range(4):
            f.write("Best Recall@{}: {:.4f}\n".format(10**i, best_recall[i] * 100))

if args.loss == 'MS':
    with open('{}/{}_{}_{}_thresh{}_epsilon{}_scalepos{}_scaleneg{}.txt'.format(LOG_DIR, args.dataset, args.model, args.loss, args.thresh, args.epsilon, args.scale_pos, args.scale_neg), 'w') as f:
        for i in range(4):
            f.write("Best Recall@{}: {:.4f}\n".format(10**i, best_recall[i] * 100))

if args.loss == 'SoftTriple':
    with open('{}/{}_{}_{}_margin{}_lamda{}_gamma{}_tau{}.txt'.format(LOG_DIR, args.dataset, args.model, args.loss, args.margin, args.lamda, args.gamma, args.tau), 'w') as f:
        for i in range(4):
            f.write("Best Recall@{}: {:.4f}\n".format(10**i, best_recall[i] * 100))